...
const AccordionItemWrapper = styled.div<{ $active: boolean }>`
...
const AccordionTriggerInfo = styled.div<{ $shifted?: boolean }>`
  margin-left: ${props => (props.$shifted ? '8px' : '16px')};
  flex-grow: 1;
  transition: margin-left 0.5s ease;
`;

const AccordionTriggerIcon = styled.div<{ rotated: boolean }>`
  display: flex;
  align-items: center;
...
const ItemCategoryLabel = styled.span`
  display: block;
  font-size: 10px;
  font-weight: 600;
...
const ItemCategoryText = styled.h3<{ $active?: boolean }>`
...
const AccordionTriggerImageWrapper = styled.div<{ $color: string | null; $hide?: boolean }>`
  width: 48px;
  height: 48px;
  border-radius: var(--kv-card-border-radius);
  overflow: hidden;
...
const AccordionContentInner = styled.div<{ $isOpen: boolean }>`
...
const GridOption = styled.div<{ $selected?: boolean }>`
  position: relative;
  cursor: pointer;
  border-radius: 20px;
  padding: 8px;
  border: 1px solid ${props => (props.$selected ? 'var(--kv-color-accent)' : 'transparent')};
...
const OptionImage = styled.div<{ $color: string | null }>`
  width: 100%;
  aspect-ratio: 1;
  border-radius: 14px;
  overflow: hidden;
...
const SelectedBadge = styled.div<{ $show: boolean }>`
  position: absolute;
  top: 14px;
  right: 14px;
  width: 18px;
